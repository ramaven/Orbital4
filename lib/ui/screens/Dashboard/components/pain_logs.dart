...
int twoWeeksAgoMonth = int.parse(twoWeeksAgo.toString().substring(5, 7));
String twoWeeksAgoString = twoWeeksAgo.toString().substring(5, 7) +
    "." +
    twoWeeksAgo.toString().substring(8, 11);
double twoWeeksAgoDouble = double.parse(twoWeeksAgoString);
int currentMonth = DateTime.now().month;
int currentDay = DateTime.now().day;
double currentDateDouble = getDate(currentDay, currentMonth);
              ...
              child: SizedBox(
                ...
                child: LineChart(
                  LineChartData(
                      ...
                      minY: 0,
                      maxY: 10,
                      minX: twoWeeksAgoDouble,
                      maxX: currentDateDouble,
  ...
  List<LineChartBarData> createLineData(
    ...
    for (int i = 0; i < painLogsFirebase.length; i++) {
      ...
      if (bodyPart == topBodyPart && dateDateTime.compareTo(twoWeeksAgo) > 0) {
        ...
        // This gives us 'mm.dd'
        String formattedDateString = dateDateTime.toString().substring(5, 7) +
            "." +
            dateDateTime.toString().substring(8, 11);

        // Convert string to double. Note this loses the padded 0s infront if any
        double dateDouble = double.parse(formattedDateString);

        // Add data to flSpots array
        // List<flspots>.add(day, level) --> returns List<FLSpots> with many plot points
        halfList.add(FlSpot(dateDouble, painLevel));
      }
    }
